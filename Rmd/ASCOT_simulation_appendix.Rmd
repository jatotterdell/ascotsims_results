...
```{r, fig.cap="Expected posterior probability treatment in domain A is effective as trial progresses.", eval=FALSE, include=FALSE}
res0$res_trt_intrm %>%
  unnest(data) %>%
...
```{r, fig.cap="Expected posterior probability treatment in domain B is effective as trial progresses.", eval=FALSE, include=FALSE}
res0$res_trt_intrm %>%
  unnest(data) %>%
  filter(grepl("b[1-9]", treatment)) %>%
  group_by(interim, treatment) %>%
  summarise(p = mean(trt_eff)) %>%
  mutate(domain = paste("Domain:", toupper(str_sub(treatment, 1, 1))),
         treatment = paste0("B", str_sub(treatment, 2, 2))) %>% 
...
```{r, fig.cap="Expected posterior probability treatment in domain B is effective as trial progresses."}
res$res_trt_intrm %>%
  unnest(data) %>%
  filter(grepl("b[1-9]", treatment)) %>%
  group_by(OR, interim, treatment) %>%
...
```{r, fig.cap="Expected posterior summary (mean and 95% CI) of treatment effect in domain B as trial progresses.", eval=FALSE, include=FALSE}
res$res_trt_intrm %>%
  unnest(data) %>%
  filter(grepl("b[1-9]", treatment)) %>%
  group_by(OR, interim, treatment) %>%
  ...
  ggplot(., aes(ss, m, colour = treatment, group = treatment)) +
  facet_wrap(~ OR) +
  geom_pointrange(aes(ymin = l, ymax = u), position = position_dodge(width = 150), size = 0.5, fatten = 0.75) +
  geom_hline(aes(yintercept = 0), linetype = 2) +
  scale_colour_manual("Treatment", values = cbp2) +
  labs(x = "Total trial sample size", y = "Expected posterior summary",
       colour = "Treatment") +
...
```{r, fig.cap="Expected posterior probability treatment in domain C is effective as trial progresses.", eval=FALSE, include=FALSE}
res0$res_trt_intrm %>%
  unnest(data) %>%
  filter(grepl("c[1-9]", treatment)) %>%
  group_by(interim, treatment) %>%
  summarise(p = mean(trt_eff)) %>%
  mutate(domain = paste("Domain:", toupper(str_sub(treatment, 1, 1))),
         treatment = paste0("C", str_sub(treatment, 2, 2))) %>% 
...
```{r, fig.cap="Expected posterior probability treatment in domain C is effective as trial progresses."}
res$res_trt_intrm %>%
  unnest(data) %>%
  filter(grepl("c[1-9]", treatment)) %>%
  group_by(OR, interim, treatment) %>%
...
<!-- ```{r, fig.cap="Expected mean and lower and upper 95% credible interval."} -->
<!-- res0$res_par %>% -->
<!--   unnest(data) %>% -->
<!--   filter(grepl("b", parameter)) %>% -->
<!--   group_by(parameter) %>% -->
<!--   summarise(lb = mean(par_lb), ub = mean(par_ub), m = mean(par_mu)) %>% -->
<!--   ungroup() %>% -->
<!--   gather(measure, value, -parameter) %>% -->
<!--   ggplot(., aes(parameter, value, group = measure, colour = measure)) + -->
<!--   geom_point() + -->
<!--   geom_hline(aes(yintercept = 0), linetype = 2) + -->
<!--   geom_hline(aes(yintercept = qlogis(0.2)), linetype = 3) + -->
<!--   scale_colour_manual("Treatment", values = cbp2) + -->
<!--   labs(x = "Model parameter", y = "Expected value", -->
<!--        colour = "Measure") + -->
<!--   theme(legend.position = "top") -->
<!-- ``` -->
...
<!--   unnest(data) %>% -->
<!--   filter(grepl("b", parameter)) %>% -->
<!--   group_by(OR, parameter) %>% -->
<!--   summarise(lb = mean(par_lb), ub = mean(par_ub), m = mean(par_mu)) %>% -->
<!--   ungroup() %>% -->
<!--   gather(measure, value, -OR, -parameter) %>% -->
<!--   ggplot(., aes(OR, value, group = measure, colour = measure)) + -->
<!--   facet_wrap(~ parameter) + -->
<!--   geom_point() + -->
<!--   geom_hline(aes(yintercept = 0), linetype = 2) + -->
<!--   geom_hline(aes(yintercept = qlogis(0.2)), linetype = 2) + -->
<!--   scale_colour_manual("Treatment", values = cbp2) + -->
<!--   labs(x = "Effect size", y = "Expected value", -->
<!--        colour = "Measure") + -->
<!--   theme(legend.position = "top") -->
<!-- ``` -->
