...
header-includes:
  ...
  - \fancyfoot[LO]{Version 0.2, August, 2020}
...
\begin{center}
    ...
    0.2 & August 2020 & JT & Update to align with core protocol and incorporate comments from SWG \\ 
    \hline
    \end{tabular}
\end{center}
```
...
Currently, adjustment will be made for dexamethasone and remdesivir.
...
Treatment effect heterogeneity will be explored for the following variables as measured at baseline across all domains:
  ...
  * severity of illness: required supplemental oxygen at time of randomisation or oxygen saturation less than 94% at room air
  * age: $<65$ years of age or $\geq 65$ years of age
  * standard of care: receiving remdesivir prior to randomisation or will be provided as part of care
  * standard of care: receiving dexamethasone prior to randomisation or to be provided as part of care
...
Patients are randomised to regimens which consist of one treatment from each domain.
Initially, all treatments within a domain and therefore all regimens will be allocated with equal probability.
...
The standard of care option within each domain will have a fixed allocation of $1/K^\prime_d$ where $K^\prime_d$ is the number of active interventions in the domain.
...
If a participant is eligible for at least two active interventions, the participant will be randomised amongst those eligible interventions and treated as eligible for the domain.
The participant will be included in the primary analysis and a covariate indicating their treatment specific ineligibility will be included to account for possible associations between participant factors determining ineligibility for a particular intervention and the outcome.
Each intervention in will have it's own ineligibility effect where necessary.
...
At each analysis, the posterior probability that an intervention is futile (insufficiently better than standard of care, see Section \@ref(treatment-contrasts) or insufficiently better than another reference treatment) will be compared to a threshold of 0.95.
...
The two contrasts of primary interest are the comparison of each treatment with the reference treatment $(\psi_{k1}(\Delta))$, and where relevant, the comparison of combination of within domain treatments versus either given alone. For example, if treatment option $l$ is the combination of treatment options $k_1$ and $k_2$ given together then the contrasts $\psi_{l,k_1}^d(-\log(1.1))$ and $\psi_{l,k_2}^d(-\log(1.1))$ may be of interest as additional futility checks for intervention $l$.
...
When a statistical decision has been made this will be referred to the DSMB.
Subject to review by the DSMB a platform conclusion may be declared.
